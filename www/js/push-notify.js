    ...
    const tick = () => {
      ...
      if (body && Notification.permission === 'granted') {
        try {
          ...
          // Foreground in-app reminder fallback (always shown even if OS notifications are blocked)
          try {
            const el = document.getElementById('nexusInAppNotif');
            ...
            if (el) {
              ...
              line.innerHTML = `<span class="text-slate-600">[${new Date().toLocaleTimeString()}]</span> <b>[REMINDER]</b> ${body}`;
              container.appendChild(line);
              container.scrollTop = container.scrollHeight;
            }
          } catch {}

          // Best-effort OS notification (may be blocked in WebView)
          new Notification(title || 'Nexus Reminder', { body });
        } catch {}
      }
